...
import { findBead } from '../content/beads'
...
import { BeadGlyph, BraceletMaker } from '../components/BraceletMaker'
import { fireConfetti } from '../components/Confetti'
import { SayIt } from '../components/SayIt'
import { awardNewBadges } from '../store/badges'
import { awardBeads, awardItem } from '../store/collection'
...
type BoxTab = 'boxes' | 'cards' | 'bracelets' | 'tickets'
...
/** "+3 beads" chip row with a tiny SVG of each bead's shape (see box-beads testid). */
function BeadDropRow({ beadIds }: { beadIds: string[] }) {
  if (beadIds.length === 0) return null
  return (
    <div data-testid="box-beads" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexWrap: 'wrap', justifyContent: 'center' }}>
      <strong>+{beadIds.length} beads</strong>
      <div style={{ display: 'flex', gap: '0.15rem' }}>
        {beadIds.map((id, i) => {
          const bead = findBead(id)
          if (!bead) return null
          return (
            <svg key={`${id}-${i}`} viewBox="0 0 32 32" width={20} height={20}>
              <BeadGlyph bead={bead} cx={16} cy={16} r={12} />
            </svg>
          )
        })}
      </div>
      ...
      >
        ...
        {result.duplicate && (
          <p style={{ margin: 0, background: 'var(--cc-bg)', borderRadius: '0.75rem', padding: '0.5rem', fontWeight: 700 }}>
            You already have this one! It turns into {result.beadIds.length} beads ✨
          </p>
        )}

        <BeadDropRow beadIds={result.beadIds} />
...
function BoxesTab({ onResultShown }: { onResultShown: () => void }) {
  ...
  function openBox(tier: Tier) {
    ...
    setTimeout(() => {
      ...
      const { item, rarity, beadIds, duplicate } = rollBoxContents(tier, doc.collection.items, lastItemId)
      ...
      setResult({ tier, item, rarity, beadIds, duplicate, ticket: ticketEntry })
      setOpening(null)
    }, 1500)
  }
