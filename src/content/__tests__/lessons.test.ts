...
describe('hold structure', () => {
  ...
  it('every namedAlgId referenced anywhere (hold-level or step-level) exists in NAMED_ALGS', () => {
    for (const lesson of LESSON_LIST) {
...
describe('mission structure', () => {
  ...
  it('every mission has 1-4 steps and 3-6 estimated minutes', () => {
    for (const lesson of LESSON_LIST) {
      for (const mission of lesson.missions) {
  ...
  it('every stickering used is a real cubing.js preset', () => {
    ...
    for (const lesson of LESSON_LIST) {
      for (const mission of lesson.missions) {
        if (mission.look.stickering) expect(allowed.has(mission.look.stickering), `${lesson.id}/${mission.id}`).toBe(true)
        if (mission.check.stickering) expect(allowed.has(mission.check.stickering), `${lesson.id}/${mission.id}`).toBe(true)
        for (const step of mission.steps) {
          if (step.kind === 'do' && step.stickering) {
            expect(allowed.has(step.stickering), `${lesson.id}/${mission.id}`).toBe(true)
          }
        }
      }
    }
...
function missionCards(id: HoldId): LearnCard[] {
  ...
  for (const m of LESSONS[id].missions) {
    out.push(m.look)
    for (const s of m.steps) if (s.kind === 'do') out.push(s)
  }
  return out
}
