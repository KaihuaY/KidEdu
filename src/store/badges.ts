 ...
 * unearned to earned (mission completions, streak bests, take counts,
 * 3-star days, and the badges already on record) - lets useBadgeAwards skip
 * re-checking on unrelated re-renders, e.g. a piano level meter tick.
 */
function badgeSignature(doc: ProgressDoc): string {
  const kid = doc.profiles.kid
  const holdsSignature = Object.keys(kid.holds)
    .sort()
    .map((id) => {
      const hold = kid.holds[id]
      const doneCount = Object.values(hold.missions ?? {}).filter((m) => m.completedAt).length
      return `${id}:${hold.masteredAt ?? 0}:${doneCount}`
    })
    .join(',')
  const tripleStarDays = Object.values(doc.piano.days).filter((d) => d.parentStars === 3).length

  return [
    kid.streak.best,
    doc.piano.streak.best,
    holdsSignature,
    doc.piano.takes.length,
    tripleStarDays,
